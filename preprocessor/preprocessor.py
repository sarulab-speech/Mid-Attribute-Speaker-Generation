...
class Preprocessor:
    ...
    def build_from_path(self):
        ...
        random.shuffle(out)
        _ = [random.shuffle(r) for r in out]
        out = [[r_ for r_ in r if r_ is not None] for r in out]

        # Write metadata
        with open(os.path.join(self.out_dir, "train.txt"), "w", encoding="utf-8") as f:
            for spk in out:
                for m in spk[:int(len(spk)*(1-self.val_size-self.test_size))]:
                    f.write(m + "\n")
        with open(os.path.join(self.out_dir, "val.txt"), "w", encoding="utf-8") as f:
            for spk in out:
                for m in spk[int(len(spk)*(1-self.val_size-self.test_size)):int(len(spk)*(1-self.test_size))]:
                    f.write(m + "\n")
        with open(os.path.join(self.out_dir, "test.txt"), "w", encoding="utf-8") as f:
            for spk in out:
                for m in spk[int(len(spk)*(1-self.test_size)):]:
                    f.write(m + "\n")
